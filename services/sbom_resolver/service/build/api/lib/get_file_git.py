...
# python3 get_file_git.py  --src /tmp/alpine/src --resolved  /tmp/resolved.json  --output internal  --debug --mode internal 
   ...
   for repo in sbom['metadata']['aggregator']['alpine']['aports']['repo']: 
     ...
     if 'source' in sbom['metadata']['aggregator']['alpine']['aports']['repo'][repo]: 
        ...
        for package in source: 
          index = package['index']
          if 'aggregate' in sbom['dependencies'][index]  and 'source' in sbom['dependencies'][index]['aggregate'] and 'internal' in sbom['dependencies'][index]['aggregate']['source']: 
            ...
            if src_type in ['all','code']: 
             for code in src['code'] : 
              ...
              try: 
                ...
                sys.stderr.write("    **********Problem with component [%s] See TODO.txt\n" %  package['package']) 
              else: 
                if debug: 
                   sys.stdout.write("Internal code copied: %s\n" % dstpath) 
            ...
            if src_type in ['all','patch']: 
             for code in src['patch'] : 
              ...
              try: 
                ...
                if debug: 
                   sys.stdout.write("Internal patch copied: %s\n" % dstpath) 
...
def get_external_code(sbom, srcdir, dstdir, mode,src_type,  debug ): 
   ...
   for repo in sbom['metadata']['aggregator']['alpine']['aports']['repo']: 
     ...
     if 'source' in sbom['metadata']['aggregator']['alpine']['aports']['repo'][repo]: 
        ...
        for package in source: 
          index = package['index']
          if 'aggregate' in sbom['dependencies'][index]  and 'source' in sbom['dependencies'][index]['aggregate'] and 'external' in sbom['dependencies'][index]['aggregate']['source']: 
             ...
             for code in src['code'] : 
              if 'generic' in code['remote']['type']: 
                ...
                try: 
                  ...
                  sys.stderr.write("    **********Problem to download [%s] from [%s]\n" %  (package['package'],url)) 
                else: 
                  if debug: 
                   sys.stdout.write("External code downloaded: %s\n" % url) 
            if src_type in ['all','patch']: 
             for code in src['patch'] : 
              if 'generic' in code['remote']['type']: 
                path = code['local']['path']
                ...
                try: 
                  ...
                  if debug: 
                   sys.stdout.write("External patch downloaded: %s\n" % url) 
...
def main():
    ...
    if  args.resolved: 
       ...
       try: 
        ...
        sys.stderr.write("Unable to find tag and architecture in main repo for SBOM %s\n" % args.resolved) 
        sys.exit(1) 
       else: 
    ...
    if mode in  ['all']: 
       get_internal_code(resolved_sbom, args.src, dstdir, mode,src_type,  args.debug )
       get_external_code(resolved_sbom, args.src, dstdir, mode,src_type,  args.debug )
